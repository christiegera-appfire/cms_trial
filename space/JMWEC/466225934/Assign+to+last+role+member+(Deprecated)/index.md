...
| Project role | Select user → **Last role member** | Set the new **Project Role** field to the same value as the obsolete post function configuration. |
| **Note**: The [Assign issue(s)](/cms_trial/space/JMWEC/1133772861/Assign+issue(s)/) post function has expanded capabilities for determining issue assignment. Review **Assignment behavior** in the new configuration and set as needed. |
...
8. Select the project role from the `Project Role` drop-down.
