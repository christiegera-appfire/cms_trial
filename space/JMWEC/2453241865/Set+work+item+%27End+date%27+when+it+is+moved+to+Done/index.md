# Set work item 'End date' when it is moved to Done

| **Goal** | When a work item is moved to the ‘Done’ status, update its End date to keep the Gantt chart view in BigPicture up to date and remove manual field updates. |
| --- | --- |
| **Scenario** | [BigPicture](https://marketplace.atlassian.com/apps/1212259/bigpicture-project-management-ppm?hosting=cloud&tab=overview) is a flexible PPM tool for Jira to help you manage projects, programs, and portfolios in one place, at any scale. You want to make sure your BigPicture Gantt charts are as accurate as possible, so in an effort to guarantee complete and accurate data, you want to automatically set the End date of your work items to the date they are transitioned to ‘Done’. Additionally, you want to verify that the End date of a work item cannot be set unless the work item is in a status other than ‘To Do’, so that the date cannot be set before work begins, but it can be reset manually if needed. |
...
| **Baseline** | - Transitioning an issue to ‘Done’ will always set the End date field, overwriting any existing value. |
...
In this step, you will add a post function to the transition that points to your **Done** status. If you have multiple transitions leading to **Done**, you should repeat these steps for each transition.
...
**Note**: Following the configuration below will set the **End date** field for any work item that is transitioned. If you want to limit this behavior to a specific type of work item, use the [Conditional execution](/cms_trial/space/JMWEC/465504849/Set+issue+fields/) option.
...
The End dates for your work items will now be updated automatically, keeping your Gantt chart current and accurate!

![Transition Issue post function configuration screen](/cms_trial/assets/b773abb7-25ae-4381-90f6-bd1c4a65a310.png)