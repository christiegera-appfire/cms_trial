...
The **Status Entered Time** custom field is a calculated field that displays when the issue was transitioned to its current status. It is a date/time field, and it is read-only.
...
## Configure a new ‘Status Entered Time’ field
...
2. Click **Next**.
3. From the **Select Status** menu, set the status value or values which will trigger the custom field to update. When selecting multiple status values, a change to any of the selected values will trigger an update to the custom field.
...
![Status Entered Time Screen](/cms_trial/assets/80ed4c66-d1fb-49fb-8c0a-5c4571fd6320.png)
