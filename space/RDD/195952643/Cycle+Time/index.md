...
Cycle time measures the amount of time it takes for a task to move **from the moment work begins until completion**, including any time spent in stages such as coding, testing, and deployment. Teams gain a clear picture of the efficiency of their processes.

It’s also helpful for identifying bottlenecks in the workflow that may be causing delays or slowing down the delivery of work.

You can also measure the Lead Time. Refer to [Lead Time](/cms_trial/space/RDD/195952856/Lead+Time/) for more information.

![Dashboard Hub Cycle time](/cms_trial/assets/b0c808a0-ab3d-4f01-bbd9-da0193019a76.png)
...
- **Assignees**. Calculates cycle time forthe **selected assignees**. Select the issue types you want to exclude from the calculation.
- **Projects**. Calculating cycle time for **the selected projects**. Select the issue types you want to exclude from the calculation.
- **Epics**. Calculates cycle time for child issues in the selected epics, not the epics themselves. Select the issue types you want to exclude from the calculation.
- **Filters/Custom JQL**. Calculates cycle time for returned issues by the query.

### Statuses included in the Cycle Time

Issues move through a workflow made up of *statuses* and *transitions*. Cycle time indicates the **time spent working on an issue**, beginning when the issue moves to In Progress in progress, and ending where it moves to the Done done Status Category):

![Jira workflow configuration showing statuses mapped to the In Progress status category.](/cms_trial/assets/d661c609-1b17-4cf9-b2f1-d724d758cf95.png)

![Jira workflow configuration showing statuses mapped to the Done status category.](/cms_trial/assets/d507fc2e-d594-421f-9d1c-36b155c1bdb3.png)
...
- 🟢 Green ring: Cycle time is within the range 0% to 100% of your goal.
- 🟡 Yellow circle: Cycle time is within the range 100% to 150% of your goal.
- 🔴 Red circle: Cycle time is within the range 150% to 200% (or higher) of your goal.

![Cycle time goal indicator showing green, yellow, and red performance rings.](/cms_trial/assets/9d7deb38-4beb-40e8-b721-b307a142c7f9.jpg)

### List individual elements

Select this option to list the different elements individually to display their times. The following example displays all the projects in the bottom part of the gadget:
...
- **Datasource**: Select where the source Jira instance is installed.
- **Reporting by**: Select the element of work you want to measure the cycle time for: Assignees, Projects, Epics or Filters/Custom JQL
- **Statuses**: Measure the cycle time:
...
- **Goal for this metric**: set the target cycle time.
- **Time is calculated by means of**:

  - **Average**: The arithmetic mean sums the values of the grouped items and divides the result by the total number of items.
  - **Median**: Returns the middle value of the range of items of the group/field. If the number of items is even, then, the mean of the two values in the middle is returned.
- **List all the individual report elements**: Display in the gadget all the times of the items individually.

Indicate if you want to use the current settings for all the compatible gadgets in the dashboard. This option saves time by applying the current settings to all compatible gadgets.

## FAQ

Why is a Goal field required? Why not just show the raw times?

The **Goal** field in the Lead Time and Cycle Time gadgets allows you to set a target for their time data, which can then be displayed on a graph along with the actual time data. This field is useful because it provides a visual representation of how well the team is meeting their time target and can help them identify when they are falling behind.

Without a defined target, the data presented in the gadget may not provide much actionable insight. For example, if the graph simply shows the average lead time for work items over time, it may be difficult to determine whether that time is reasonable or not. Additionally, without a target, the team may not have a clear goal to work towards or a way to measure their progress. This can make it more difficult to identify areas for improvement in their delivery process and make meaningful changes. By setting a time target, the team has a clear goal to work towards and can more easily track their progress toward that goal. This can help identify areas for improvement and continuously optimize their delivery process.
...
We are constantly expanding our catalog of [Dashboard gadgets - KPIs and metrics](/cms_trial/space/RDD/146309915/Dashboard+gadgets+-+KPIs+and+metrics/) and [Product and data connections](/cms_trial/space/RDD/146310010/Product+and+data+connections/). If you want us to expedite a specific one, visit our [Help and support](/cms_trial/space/RDD/146309984/Help+and+support/).
